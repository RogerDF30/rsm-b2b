...
Replacement for the Magento storefront at `b2b.rsmus.companystore.gifts`.
...
**Read this before showing the site to anyone at RSM.**
...
The RSMB012862 order shows the client running 5% on a ₹1,953 duffle bag and 18%
...
```
git init && git add -A && git commit -m "RSM B2B store"
git remote add origin git@github.com:<you>/rsm-b2b.git
...
| `SENDER_ALIAS` | `rsm@companystore.io` |
| `SITE_URL` | `https://<you>.github.io/rsm-b2b` |
...
The old system used one static code, `RSM$2025JN`, hardcoded in `WebApp.js`,
...
Order IDs continue the Magento sequence from `RSMB013682`. The last live Magento
order was `RSMB013681` on 15 June 2026.
...
**Mail deliverability.** Approvals go to `@rsmus.com`, a Microsoft tenant.
Confirm SPF and DMARC alignment for the `rsm@companystore.io` alias before
go-live.
...
4. The full RSM user list. The order export yields only 11 requesters.
